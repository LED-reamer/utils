...
font_t font_create_from_memory(allocator_t* allocator, void* buffer, size_t buffer_size, float pixel_size){
	...
	font.ascent = roundf(ascent * font_to_pixel_scale);
	font.descent = roundf(descent * font_to_pixel_scale);
	font.line_height = roundf((ascent - descent + line_gap) * font_to_pixel_scale);
	...
	for (uint32_t i = 0; i < (uint32_t)info.numGlyphs; i++){
		...
		stbtt_GetCodepointHMetrics(&info, i, &advance_width, &left_size_bearing);
