...
void gpu_draw_indexed(gpu_context_t* ctx, pipeline_t* pipeline, mesh_t* mesh, size_t first_index, size_t num_indecies){
	...
	if(mesh->index_size == sizeof(uint16_t)) index_size = SDL_GPU_INDEXELEMENTSIZE_16BIT;
	else if(mesh->index_size == sizeof(uint32_t)) index_size = SDL_GPU_INDEXELEMENTSIZE_32BIT;
	else{
...
pipeline_t pipeline_create(gpu_context_t* ctx, shader_t* shader, size_t vertex_size, attribute_e vertex_attribs[], size_t num_attribs) {
	...
	for(size_t i = 0; i < num_attribs; i++) {
		...
		switch((vertex_attribs[i])){
			case ATTRIBUTE_FLOAT1: offset+=sizeof(float)*1; sdl_attribs[i].format = SDL_GPU_VERTEXELEMENTFORMAT_FLOAT; break;
			case ATTRIBUTE_FLOAT2: offset+=sizeof(float)*2; sdl_attribs[i].format = SDL_GPU_VERTEXELEMENTFORMAT_FLOAT2;break;
			case ATTRIBUTE_FLOAT3: offset+=sizeof(float)*3; sdl_attribs[i].format = SDL_GPU_VERTEXELEMENTFORMAT_FLOAT3;break;
			case ATTRIBUTE_FLOAT4: offset+=sizeof(float)*4; sdl_attribs[i].format = SDL_GPU_VERTEXELEMENTFORMAT_FLOAT4;break;
