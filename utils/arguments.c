...
argument_t* __get_argument_from_string(argument_parser_t* arg_parser, const char* flag_name_or_alias, bool getter) {
	...
	for (size_t i = 0; i < array_len(arg_parser->arguments); i++) {
		if (0 == strncmp(flag_name_or_alias, arg_parser->arguments[i].name, strlen(arg_parser->arguments[i].name)) && strlen(flag_name_or_alias) == strlen(arg_parser->arguments[i].name)) {
			return &arg_parser->arguments[i];
		}

		if (0 == strncmp(flag_name_or_alias, arg_parser->arguments[i].alias, strlen(arg_parser->arguments[i].alias)) && strlen(flag_name_or_alias) == strlen(arg_parser->arguments[i].alias)) {
			return &arg_parser->arguments[i];
		}
	}
	return NULL;
